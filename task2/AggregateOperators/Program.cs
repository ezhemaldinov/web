...
namespace AggregateOperators
{
    class Program
    {
        ...
        {
            LinqSamples data = new LinqSamples();
            ...
            Console.WriteLine();
            var numQuary1 = customers.Where(customer => customer.Region == "WA");
            foreach (var cuQ in numQuary1)
            {
                Console.WriteLine("ID: {0} | Company Name: {1} | Region: {2} ", cuQ.CustomerID, cuQ.CompanyName, cuQ.Region);
            }
            ...
            var numQuary2 = digits.Where((num, index) => num.Length < index);
            foreach (var diQ in numQuary2)
            {
            ...
            var numQuary3 = from num in digits where num == num.ToUpper() select num;
            foreach (var diQ in numQuary3)
            {
                Console.WriteLine(diQ);
            }

            var numQuary4 = from num in digits where (num[0] != num.ToLower()[0]) && (num != num.ToUpper()) select num;
            foreach (var diQ in numQuary4)
            {
                Console.WriteLine(diQ);
            }
            ...
            var minProductCost = products.Min(product => product.UnitPrice);
            Console.WriteLine("Minimal product cost : {0}", minProductCost);

            Console.WriteLine();
            Console.WriteLine("Task 9");
            Console.WriteLine();

            var maxProductCost = products.OrderByDescending(num => num.UnitPrice).First();
            Console.WriteLine("Maximal cost product's : {0}", maxProductCost);

            Console.WriteLine();
            Console.WriteLine("Task 10");
            Console.WriteLine();

            var waCost = customers.Where(customer => customer.Region == "WA").SelectMany(customersOwner => customersOwner.Orders.Select(f => f.Total)).Sum();
            Console.WriteLine("WA customer's total cost : {0}", waCost);

            Console.WriteLine();
            Console.WriteLine("Task 11");
            Console.WriteLine();

            var maxOrder = customers.OrderByDescending(sort => sort.Orders,new compareCustomerOrders()).Take(5);
            Console.WriteLine("Companys get max cost of order: ");
            foreach (var element in maxOrder)
            {
                Console.WriteLine("Company Name: {0}", element.CompanyName);
            }
           ...
           var maxSumOfOrder = customers.OrderByDescending(sort => sort.Orders, new compareSumOfOrders()).Take(5);
           Console.WriteLine("Companys get max summary cost of order: ");
           foreach (var element in maxSumOfOrder)
           {
               Console.WriteLine("Company Name: {0}", element.CompanyName);
           }
          ...
          var intersectTopCustomer = maxOrder.Intersect(maxSumOfOrder, new CustomerComparer());
          foreach (var iTC in intersectTopCustomer)
          {
              Console.WriteLine("Name {0}", iTC.CompanyName);
          }
        ...
        class CustomerComparer : IEqualityComparer<Customer>
        {
            public bool Equals(Customer x, Customer y)
            {
                ...
                return x.CustomerID == y.CustomerID && x.CompanyName == y.CompanyName;
             }

            public int GetHashCode(Customer customer)
            {
                if (Object.ReferenceEquals(customer, null)) return 0;

                int hashCustomerName = customer.CompanyName == null ? 0 : customer.CompanyName.GetHashCode();

                int hashCustomerID = customer.CustomerID.GetHashCode();
        ...
        public class compareSumOfOrders : IComparer<Order[]>
        {
            public int Compare(Order[] obj1, Order[] obj2)
            {
                int Sum1 = 0;
                int Sum2 = 0;
                foreach (var value in obj1)
                {                 
                        Sum1 += Convert.ToInt32(value.Total);
                }
                foreach (var value in obj2)
                {
                        Sum2 += Convert.ToInt32(value.Total);
                }
        ...
        public class compareCustomerOrders : IComparer<Order[]>
        {
            public int Compare(Order[] obj1, Order[] obj2)
            {
                int max1 = 0;
                ...
                foreach(var value in obj1)
                {
                    if (value.Total > max1)
                    {
                        max1 = Convert.ToInt32(value.Total);
                    }
                }
                foreach (var value in obj2)
                {
                    if (value.Total > max2)
                    {
                        max2 = Convert.ToInt32(value.Total);
                    }
                }
        ...
        public class Product
        {
            ...
            {
                return string.Format("ProductName: {0}, Category: {1}, UnitPrice: {2}, UnitsInStock: {3}", ProductName, Category, UnitPrice, UnitsInStock);
            }
        }
        ...
        public class Order :IComparable<Order>
        {
            ...
            public override string ToString()
            {
                return string.Format("Total: {0}, OrderDate: {1}", Total, OrderDate);
            }
        }
        ...
        public class Customer
        {
            ...
            public override string ToString()
            {
                //return string.Format("CompanyName: {0}, Address: {1}, City: {2}, Region: {3}, PostalCode: {4}, Country: {5}, Phone: {6}, Fax: {7}, Orders: {8}", CompanyName, Address, City, Region, PostalCode, Country, Phone, Fax, Orders);

                StringBuilder sb = new StringBuilder();
                sb.AppendLine("Навзвание компании: " + CompanyName);
                sb.AppendLine("Телефон: " + Phone);
                sb.AppendLine("Страна: " + Country);
                return sb.ToString();
            }
        }
